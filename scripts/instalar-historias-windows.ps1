# =============================================================
# Triwer Skills — Instalador do historias-triwer (Windows/PowerShell)
# =============================================================
...
$HISTORIAS_DIR = "$SKILLS_DIR\historias-triwer"
$VERSION_FILE = "$HISTORIAS_DIR\VERSION"
...
Write-Host ""
Write-Color "╔══════════════════════════════════════╗" "Blue"
Write-Color "║   Triwer Skills — historias-triwer   ║" "Blue"
Write-Color "╚══════════════════════════════════════╝" "Blue"
Write-Host ""
...
try {
    $REMOTE_VERSION = (Invoke-WebRequest -Uri "$REPO/historias-triwer/VERSION" -UseBasicParsing).Content.Trim()
} catch {
...
New-Item -ItemType Directory -Force -Path $HISTORIAS_DIR | Out-Null

# Baixar arquivos principais
Write-Color "→ Baixando historias-triwer..." "Yellow"
Download-File "historias-triwer/SKILL.md" "$HISTORIAS_DIR\SKILL.md"

# memoria.md: nunca sobrescrever se já existir
if (-not (Test-Path "$HISTORIAS_DIR\memoria.md")) {
...
Write-Host ""
Write-Host "  2. Não precisa rodar por comando — a partir daqui, sempre que"
Write-Host "     você contar um caso real numa conversa, esta skill considera"
Write-Host "     se vale oferecer cadastro"
Write-Host ""
Write-Host "  3. Para uso direto: digite /historias-triwer"
Write-Host ""
